...
def csv_to_vcf(csv_file, vcf_file):
    ...
    header.add_meta(
        ...
        items=[
            ...
            ("Number", "1"),
            ("Type", "String"),
            ("Description", "Plasmy Reports (Homo/Hetero)"),
        ],
    )
    header.add_meta(
        "INFO",
    ...
    with (
        ...
        for row in reader:
            ...
            info_fields = {
                ...
                "AminoAcidChange": row["Amino\xa0AcidChange"],
                "Plasmy": row["Plasmy Reports(Homo/Hetero)"],
                "Status": row["Status"],
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python convert_to_vcf.py input.csv output.vcf")
        sys.exit(1)
