...
usage: mity call [-h] [-d] [--reference {hs37d5,hg19,hg38,mm10}]
                 ...
                 [--out-folder-path OUT_FOLDER_PATH] [--region REGION]
                 [--bam-file-list REGION]
                 bam [bam ...]

positional arguments:
  bam                   BAM files to run the analysis on.
...
options:
  ...
  --normalise           Normalise the resulting VCF
  ...
  --bam-file-list REGION
                        A text file of BAM files to be processed. The path to
                        each file should be on one row per Region of MT genome
                        to call variants in. If unset will call variants in
                        entire MT genome as specified in BAM header. Default:
                        Entire MT genome.
```

## Normalise
```bash
usage: mity normalise [-h] [-d] -o OUTFILE [--allsamples] [--p P]
                      [--reference {hs37d5,hg19,hg38,mm10}]
                      vcf
...
usage: mity merge [-h] --mity_vcf MITY_VCF --nuclear_vcf NUCLEAR_VCF
                  [--prefix PREFIX] [--reference {hs37d5,hg19,hg38,mm10}] [-d]
