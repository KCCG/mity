...
config.read(MityUtil.get_mity_dir() + "/config.ini")
...
def check_vcf_merge_compatibility(mity_vcf, hc_vcf):
    ...
    """
    m = MityUtil.vcf_get_mt_contig(mity_vcf)
    h = MityUtil.vcf_get_mt_contig(hc_vcf)
    return m == h
