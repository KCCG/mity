...
def split_header_variants(vcf):
    ...
    if not (col_names[0] == "#" and col_names[1] != "#"):
        sys.exit(
            "It looks like the last line of the header is not the column " "names."
        )
...
def do_report(debug, vcf, prefix=None, min_vaf=0.0, out_folder_path="."):
    ...
    for vcf_num in range(0, len(variants)):
        v = make_table(
            variants[vcf_num], samples[vcf_num], vep_header, impacts, min_vaf
        )
    ...
    mitomap_panel_annotations = pandas.read_csv(
        get_annot_file("mitomap_panel_annotations.csv")
    )
    ...
    mitomap_panel_annotated_variants = pandas.merge(
        left=variant_df,
    ...
    gtf_mitomap_panel_annotated_variants = pandas.merge(
        left=mitomap_panel_annotated_variants,
        right=gtf_annotations,
        how="left",
        on=["CHR", "POS"],
    )
    ...
    mito_locus_gtf_mitomap_panel_annotated_variants = pandas.merge(
        left=gtf_mitomap_panel_annotated_variants,
        right=mito_locus_annotations,
        how="left",
        on=["CHR", "POS"],
    )
    ...
    mitotip_annotations["MitoTip_score"] = mitotip_annotations["MitoTip_score"].astype(
        "str"
    )
    ...
    mgrb_mitotip_trna_mito_locus_gtf_mitomap_panel_annotated_variants = pandas.merge(
        left=mitotip_trna_mito_locus_gtf_mitomap_panel_annotated_variants,
        right=mgrb_annotations,
        how="left",
        on=["CHR", "POS", "REF", "ALT"],
    )
    # for line in trna_mito_locus_gtf_mitomap_panel_annotated_variants:
    ...
    haplotype_mgrb_mitotip_trna_mito_locus_gtf_mitomap_panel_annotated_variants = (
        pandas.merge(
            left=mgrb_mitotip_trna_mito_locus_gtf_mitomap_panel_annotated_variants,
            right=haplotype_annotations,
            how="left",
            on=["CHR", "POS", "REF", "ALT"],
        )
    )
    ...
    cohort_count_table = pandas.DataFrame(
        {"count": annotated_variants.groupby(["POS", "ALT"]).size()}
    ).reset_index()
    cohort_count_table.columns = ["POS", "ALT", "COHORT_COUNT"]

    annotated_variants1 = pandas.merge(
        left=annotated_variants, right=cohort_count_table, how="left", on=["POS", "ALT"]
    )
    ...
    if len(annotated_variants1.columns.tolist()) != len(cols):
        ...
        print("")
        print(
            "annotated_variants missing"
            + str(set(annotated_variants.columns.tolist()) - set(cols))
        )
        print("")
    ...
    annotated_variants1["COHORT_COUNT"] = annotated_variants1["COHORT_COUNT"].astype(
        "int64"
    )
    ...
    documentation_df.to_excel(
        writer, sheet_name="Documentation", index=False, header=False
    )
